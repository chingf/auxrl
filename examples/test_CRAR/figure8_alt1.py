...
class MyEnv(Environment):
    """
    3-reward W Maze. Example (where X is a wall and O is a path):

    R1 XXXXX R2 XXXXX R3
    O  XXXXX O  XXXXX O
    O  XXXXX O  XXXXX O
    O  XXXXX O  XXXXX O
    OOOOOOOOOOOOOOOOOOO
    ...
    """
    VALIDATION_MODE = 0
    RIGHT_REWARD = 1; LEFT_REWARD = 0; RESET_REWARD = 2; MIDDLE_REWARD = 3
    HEIGHT = 5
    WIDTH = 7 #Must be odd
    ...
    def __init__(self, give_rewards=False, intern_dim=2, plotfig=True, **kwargs):
        ...
        self._last_reward_locations = [MyEnv.MIDDLE_REWARD, MyEnv.RIGHT_REWARD]
    ...
    def act(self, action):
        ...
        left_reward, right_reward, middle_reward, reset_reward = self.REWARD_COORDS
        if self.in_bounds(new_x, new_y):
            if ((self.x, self.y)) == middle_reward:
                if action == 3:
        ...
        if ((self.x, self.y) == left_reward) and self._reward_location == MyEnv.LEFT_REWARD:
            ...
            self._last_reward_location = [MyEnv.LEFT_REWARD, MyEnv.RESET_REWARD]
        elif ((self.x, self.y) == right_reward) and self._reward_location == MyEnv.RIGHT_REWARD:
            self.reward = 1
            self._reward_location = MyEnv.RESET_REWARD
            self._last_reward_location = [MyEnv.RIGHT_REWARD, MyEnv.RESET_REWARD]
        elif ((self.x, self.y) == middle_reward) and self._reward_location == MyEnv.MIDDLE_REWARD:
            self.reward = 1
            self._reward_location = MyEnv.RESET_REWARD
            self._last_reward_location = [MyEnv.MIDDLE_REWARD, self._last_reward_location[0]]
        elif ((self.x, self.y) == reset_reward) and self._reward_location == MyEnv.RESET_REWARD:
            self.reward = 1
            if self._last_reward_location[0] == MyEnv.RIGHT_REWARD:
                self._reward_location = MyEnv.MIDDLE_REWARD
            elif self._last_reward_location[0] == MyEnv.LEFT_REWARD:
                self._reward_location = MyEnv.MIDDLE_REWARD
            else:
                if self._last_reward_location[1] == MyEnv.RIGHT_REWARD:
                    self._reward_location = MyEnv.LEFT_REWARD
                else:
    ...
    def get_observation(self, x, y, reward_location):
        ...
        if self._show_rewards:
            for r in self.REWARD_COORDS:
                if reward_location == r:
                    obs[r[0], r[1]] = 1
