...
source_epsilon = float(sys.argv[5])
if len(sys.argv) > 6:
...
if source_epsilon < 1.0:
    source_prefix += f'_eps{source_epsilon}'

# Define source episode
if source_epsilon < 0.4:
    source_episode = 1500
elif source_epsilon < 0.6:
    source_episode = 1200
elif source_epsilon < 0.8:
    source_episode = 900
else:
    source_episode = 600

# Other set up for running experiment
if shuffle:
    source_prefix += '_shuffobs'
fname_prefix = f'transitionreval_{source_prefix}'
n_episodes = 501
n_iters = 45
...
def run(arg):
    ...
    if source_fname is None:
        ...
        print(f'Previous goal: {prev_pos_goal}')
    ...
    parameters = {
        ...
        'dset_args': {
            'layout': size_maze, 'shuffle_obs': shuffle, #'add_barrier': True
            'shuffle_states': True
            }
        }
